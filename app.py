...
class GeeTestIdentifier:
    ...
    def find_puzzle_piece_position(self):
        ...
        if self.debugger:
            ...
            cv2.line(self.background, (center_x, 0), (center_x, self.background.shape[0]), (0, 255, 0), 2)
            cv2.line(self.background, (0, center_y), (self.background.shape[1], center_y), (0, 255, 0), 2)
            _, buffer = cv2.imencode('.png', self.background)
            encoded_image = base64.b64encode(buffer).decode('utf-8')

        return {
            "position_from_left": int(position_from_left),
            "position_from_bottom": int(position_from_bottom),
...
def process_images():
    ...
    if not bg_url or not puzzle_url:
        return jsonify({"error": "Missing 'bg_url' or 'puzzle_url' query parameters."}), 400
